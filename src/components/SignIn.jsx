import React, { useState } from 'react';
import { auth } from '../firebase'; // 引入我們設定好的 Firebase auth
// ✅ 引入 Google 登入所需的新函式
import { GoogleAuthProvider, signInWithRedirect } from "firebase/auth";

// 統一樣式
const buttonStyle = {
  display: 'flex',
  alignItems: 'center',
  justifyContent: 'center',
  gap: '10px',
  width: '100%',
  padding: '10px 15px',
  border: '1px solid #dadce0',
  borderRadius: '8px',
  cursor: 'pointer',
  fontSize: '16px',
  fontWeight: '500',
  fontFamily: 'inherit',
};

const googleButtonStyle = {
  backgroundColor: '#ffffff', // Google 按鈕背景為白色
  color: '#3c4043', // Google 按鈕文字顏色
};


function SignIn() {
  const [error, setError] = useState(null);

  // ✅ 處理 Google 登入的函式 (改用 Redirect)
  const handleGoogleSignIn = async () => {
    const provider = new GoogleAuthProvider();
    setError(null);
    try {
      // 呼叫 Firebase 的重新導向登入函式
      await signInWithRedirect(auth, provider);
    } catch (error) {
      setError("Google 登入失敗: " + error.message);
      console.error("Google sign-in error", error);
    }
  };

  return (
    <div>
      <h2 style={{ textAlign: 'center' }}>登入系統</h2>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', alignItems: 'center', marginTop: '30px' }}>
        
        {/* Google 登入按鈕 */}
        <button 
          type="button" 
          onClick={handleGoogleSignIn} 
          style={{...buttonStyle, ...googleButtonStyle}}
        >
          <svg width="18" height="18" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48">
            <path fill="#FFC107" d="M43.611,20.083H42V20H24v8h11.303c-1.649,4.657-6.08,8-11.303,8c-6.627,0-12-5.373-12-12c0-6.627,5.373-12,12-12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C12.955,4,4,12.955,4,24s8.955,20,20,20s20-8.955,20-20C44,22.659,43.862,21.35,43.611,20.083z"></path>
            <path fill="#FF3D00" d="M6.306,14.691l6.571,4.819C14.655,15.108,18.961,12,24,12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C16.318,4,9.656,8.337,6.306,14.691z"></path>
            <path fill="#4CAF50" d="M24,44c5.166,0,9.86-1.977,13.409-5.192l-6.19-5.238C29.211,35.091,26.715,36,24,36c-5.222,0-9.565-3.356-11.303-7.962l-6.571,4.819C9.656,39.663,16.318,44,24,44z"></path>
            <path fill="#1976D2" d="M43.611,20.083H42V20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.574l6.19,5.238C39.99,35.533,44,30.138,44,24C44,22.659,43.862,21.35,43.611,20.083z"></path>
          </svg>
          使用 Google 帳號登入
        </button>
      </div>
      
      {error && <p style={{ color: 'red', textAlign: 'center', marginTop: '15px' }}>{error}</p>}
    </div>
  );
}

export default SignIn;

